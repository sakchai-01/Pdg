...
# from app.database import get_user_async, create_user_async, get_user_profile_async, update_user_profile_async
...
async def report_get(request: Request): return templates.TemplateResponse("report.html", {"request": request})
...
async def report_post(request: Request, email: str = Form(...), url: str = Form(None), description: str = Form(None), type: str = Form(None)):
    ...
    return templates.TemplateResponse("report.html", {"request": request, "success": f"ขอบคุณสำหรับการแจ้งเบาะแสคุณ {email}! เราจะตรวจสอบโดยเร็วที่สุด"})

@router.get("/detect/fb", response_class=HTMLResponse)
async def detect_fb(request: Request):
    return templates.TemplateResponse("detect_fb.html", {"request": request})
